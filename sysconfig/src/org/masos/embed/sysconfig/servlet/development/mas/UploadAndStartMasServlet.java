...
@WebServlet("/uploadAndStartMasServlet")
public class UploadAndStartMasServlet extends HttpServlet {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) {
		...
		if(masFile != null && masFile.exists()) {
			...
			if (returnMasImport.contains("Archive") && returnMasImport.contains("creating") && returnMasImport.contains("inflating")) {
				SSHConnection.getDefault(user).execute(ReasoningScriptManager.EMBEDDED_MAS_START);
