package org.masos.embed.sysconfig.utils;

import net.lingala.zip4j.ZipFile;
import net.lingala.zip4j.exception.ZipException;
import org.masos.embed.sysconfig.exception.*;
...
public class FileUtils {
    ...
    public static final String BASE_PATH = System.getProperty("user.home") + File.separator + "chonIDE";
    public static final String MAS_BASE_PATH = BASE_PATH + File.separator + "Jason" + File.separator + "projects";
    ...
    public static final String ARDUINO_FILE_EXTENSION = ".ino";
    public static final String NEW_LINE = "\n";

    public static String getMasPath(String masName) {
        String path = MAS_BASE_PATH + File.separator + masName;
        File masPath = new File(path);
        int i = 1;
        while(masPath.exists()) {
            masPath = new File(MAS_BASE_PATH + File.separator + masName + i);
            i++;
        }
    ...
    public static boolean zipFolder(File folder) {
    	boolean folderWasZipped = false;
        try {
			new ZipFile(folder.getAbsoluteFile() + COMPACTED_FILE_EXTENSION).addFolder(folder);
            folderWasZipped = true;
		} catch (ZipException e) {
            throw new ErrorZippingFileException(folder.getName(), e);
		}
		
    	return folderWasZipped;
    }
